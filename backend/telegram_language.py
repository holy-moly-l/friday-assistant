...
def message_intent(text):
    ...
    match=re.match(r'^(?:(?:в|через)\s+'+APP+r'\s+)?(напиши|отправь|ответь|скинь)\b\s*(.*)$',raw,re.I|re.S)
    ...
    if match:recipient,payload=match.groups()
        ...
        match=re.match(r'^(.{1,100}?)(?:\s*:\s*|,\s*что\s+|\s+что\s+)(.+)$',body,re.I|re.S)
    ...
    if not explicit and verb.lower()=='напиши':
    ...
    payload=re.sub(r'^(?:сообщение\b\s*:?\s*|,?\s*что\s+)','',payload,flags=re.I).strip()
    # Leading comma is a command delimiter; preserve every character inside the message.
    payload=payload.lstrip(', ')
    if len(payload)>4000:return MessageIntent(error='Сообщение слишком длинное: максимум 4000 символов.')
    return MessageIntent(recipient_name(recipient),payload or None)
