...
(async()=>{
  const app=await electron.launch({executablePath:path.resolve('release/Friday-win32-x64/Friday.exe'),args:[...(process.env.FRIDAY_TEST_PROFILE?[`--user-data-dir=${process.env.FRIDAY_TEST_PROFILE}`]:[]),'--use-fake-device-for-media-stream',`--use-file-for-fake-audio-capture=${path.resolve('data/voice-sample.wav')}%noloop`],timeout:60000});
  ...
  await window.getByRole('switch',{name:'Голосовые ответы',exact:true}).click();
  await window.getByRole('button',{name:/^Новый разговор/}).click();
  await app.close();
